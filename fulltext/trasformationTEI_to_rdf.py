...
def map_node(node, parent_uri):
    ...
    for (k, v) in node.items():
        # ignore XMl ID to avoid duplicates
        if k == "{%s}id" % XML_NS:
            continue

        # If the attribute is 'sameAs', it create a OWL.sameAs triple
        if etree.QName(k).localname == "sameAs":
            # Add a owl:sameAs triple using the extern URI
            g.add((seg_uri, OWL.sameAs, URIRef(v)))
        # For other attributes, create a SKOS note
        else:
            g.add((seg_uri, SKOS.note, Literal(f"{etree.QName(k).localname}={v}")))
